...
(function () {
  ...
  window._fcBuildHTML = function (ym, r) {
    var F2 = function (v) { return (_usd && _rate) ? (v / _rate).toLocaleString("en-US", { maximumFractionDigits: 0 }) : ((typeof F === "function") ? F(v) : Math.round(v || 0).toLocaleString("en-US")); };
    ...
    h += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="background:var(--surface-2)"><th style="padding:6px 10px;text-align:left;font-size:10px;color:var(--text-3)">항목 · Item</th>'
      ...
      + '<th style="padding:6px 10px;text-align:right;font-size:10px;color:var(--text-3)">합계 (' + ((_usd && _rate) ? "USD" : "VND") + ')</th></tr></thead><tbody>'
  ...
  window.renderChasan = async function (ym, host, opts) {
    ...
    var _payload = function () { return { byDept: _live.byDept, totals: _live.totals, headByDept: _live.headByDept, totHead: _live.totHead, fx: { usd: _usd, vndPerUsd: _rate }, laborFinalized: _live.laborFinalized, allocated: _live.allocated }; };
  ...
  window.renderChasanForecastView = async function (host, ym) {
    ...
    try {
      ...
      window._csLastR = r; window._csLastYm = ym;
      body.innerHTML = '<div style="font-size:12px;color:var(--text-3);margin:0 16px 10px">기준월 · Based on ' + E(ym) + ' 확정/라이브 채산 → 다음달 손익분기 목표매출</div>'
        + '<div style="display:flex;justify-content:flex-end;align-items:center;gap:8px;margin:0 16px 10px">'
      ...
      var _fu = body.querySelector("#fcUsd"), _fr = body.querySelector("#fcRate");
      if (_fu) _fu.onchange = function () { _usd = _fu.checked; if (_usd && !(+_fr.value)) { alert("월 환율(VND/USD)을 입력하세요."); _usd = false; _fu.checked = false; return; } _fcRerender(); };
      if (_fr) _fr.onchange = function () { _rate = +_fr.value || 0; if (_usd) _fcRerender(); };
