...
(function() {
  ...
  function hrMemberOptions(selectedId) {
    ...
    var opts = '<option value="">— 연결 안 함 · Not linked —</option>';
  ...
  window.renderHrEmp = function() {
    ...
    root.innerHTML =
      ...
      + '<div class="stat-card"><div class="stat-top"><div class="stat-label">전체 직원 · Total</div></div><div class="stat-num">' + emps.length + '</div><div class="stat-sub">명 · persons</div></div>'
      + '<div class="stat-card"><div class="stat-top"><div class="stat-label">관리 대상 · HR-managed</div></div><div class="stat-num">' + managed + '</div><div class="stat-sub">hrManaged</div></div>'
      + '<div class="stat-card"><div class="stat-top"><div class="stat-label">정보 미비 · Incomplete</div></div><div class="stat-num" style="color:' + (need ? 'var(--warning)' : 'var(--success)') + '">' + need + '</div><div class="stat-sub">계약 전 보완 · fix before contract</div></div>'
      + '<div class="stat-card"><div class="stat-top"><div class="stat-label">취업규칙 · Internal Rules</div></div><div class="stat-num">' + managed + '/10</div><div class="stat-sub">' + (managed >= 10 ? '등록 의무 · filing required' : '10명 미만 · under 10') + '</div></div>'
      + '</div>'
      + '<div class="form-card" style="padding:0;overflow:hidden">'
      + '<div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">'
      + '<div style="font-size:13px;font-weight:600">직원 목록 · Employee List</div>'
      + '<div style="display:flex;gap:8px;align-items:center"><span style="font-size:11px;color:var(--text-3)">행 클릭 → 인적사항 카드 · Click a row for the profile card</span>'
      + '<button class="btn btn-dark" id="hrAddBtn" style="font-size:11px;padding:5px 12px"><i class="ti ti-plus"></i> 직원 추가 · Add Employee</button></div></div>'
      ...
      + '<th style="text-align:left;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">이름 / Name</th><th style="text-align:left;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">부서·직책 / Dept · Title</th><th style="text-align:left;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">유형 / Type</th><th style="text-align:left;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">상태 / Status</th><th style="text-align:right;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">계약급여 / Contract Salary</th><th style="text-align:left;padding:10px 14px;font-size:10px;color:var(--text-3);text-transform:uppercase">정보 / Data</th>'
      + '</tr></thead><tbody>'
      + emps.map(function(e) {
        ...
        return '<tr class="hr-emp-row" data-id="' + hrEsc(e.id) + '" style="cursor:pointer;border-bottom:1px solid var(--border)">'
          + '<td style="padding:12px 14px"><div style="font-weight:600;font-size:13px">' + hrEsc(e.nameVi || "(미입력 · not set)") + mem + '</div>'
          ...
          + '<td style="padding:12px 14px"><span class="badge ' + (e.status === "probation" ? "b-payment" : "b-done") + '" style="font-size:10px">' + (e.status === "probation" ? "수습 · Probation" : e.status === "resigned" ? "퇴사 · Resigned" : "정식 · Official") + '</span></td>'
          + '<td style="padding:12px 14px;text-align:right;font-family:var(--mono);font-size:12px">' + hrFmt(e.salary) + '</td>'
          + '<td style="padding:12px 14px">' + (m ? '<span class="badge b-payment" style="font-size:10px">미비 · Missing ' + m + '</span>' : '<span class="badge b-done" style="font-size:10px">완비 · Complete</span>') + '</td>'
          + '</tr>';
      }).join("")
      + '</tbody></table></div></div>'
      + '<p style="font-size:11px;color:var(--text-3);margin-top:10px;line-height:1.6"><b>인적사항 카드 · Profile Card</b>에서 CCCD·고용·급여를 확인하고 신분증을 첨부합니다. · Check CCCD, employment and pay, then attach ID documents.</p>';
  ...
  function hrEmpCardHtml(id) {
    var e = hrGetEmp(id);
    if (!e) return '<div class="empty-state">직원을 찾을 수 없습니다 · Employee not found</div>';
    ...
    function D(dt, dd, mono) {
      return '<div style="padding:8px 0;border-bottom:1px solid var(--border)">'
        ...
        + (dd ? hrEsc(dd) : '<span style="color:var(--warning);font-weight:600;font-size:12px">미입력 · Not set</span>') + '</div></div>';
    }
    ...
    return ''
      ...
      + '<button class="btn btn-outline" id="hrBack" style="font-size:11px;padding:5px 11px"><i class="ti ti-arrow-left"></i> 목록 · List</button>'
      + '<span style="font-size:12px;color:var(--text-3)">인사기록 · HR record ' + hrEsc(e.id) + memLine + '</span></div>'
      ...
      + (miss.length ? "⚠ 계약 전 보완 필요 · Complete before contract: " + miss.join(", ") : "✓ 인적사항 완비 · Profile complete — ready to generate contract") + '</div>'
      + '<div class="form-card" style="padding:0;overflow:hidden">'
      + '<div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">'
      + '<div style="font-size:13px;font-weight:600">인적사항 카드 · Profile Card</div>'
      + '<button class="btn btn-dark" id="hrEdit" style="font-size:11px;padding:5px 12px"><i class="ti ti-pencil"></i> 편집 · Edit</button></div>'
      ...
      + '<div><div style="font-size:16px;font-weight:600;line-height:1.3">' + hrEsc(e.nameVi || "(미입력 · not set)") + '</div>'
      ...
      + '입사 · Joined ' + hrVnDate(e.joinDate) + '<br>'
      + '<b>' + (e.status === "probation" ? "수습 · Probation" : e.status === "resigned" ? "퇴사 · Resigned" : "정식 · Official") + '</b> · '
      ...
      + D("생년월일 · Date of birth", hrVnDate(e.dob), true) + D("성별 · Gender", e.gender === "F" ? "Nữ / Female" : "Nam / Male")
      + D("CCCD 번호", e.cccd, true) + D("발급일 · Issue date", hrVnDate(e.issued), true)
      + F("발급기관 · Issuing authority", e.issuerVi) + F("영주주소 · Permanent address", e.addrVi)
      + '</div>'
      ...
      + '<div id="hrIdDocs"><div style="font-size:12px;color:var(--text-3);padding:6px 0">불러오는 중… · Loading…</div></div>'
      ...
      + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px">'
      + D("부서 · Department", e.dept) + D("직위 (VI/EN)", e.positionVi ? e.positionVi + " / " + e.positionEn : "")
      + D("구성원 연결 · Linked member", hrMemberName(e.memberId) || "—")
      + D("입사일 · Join date", hrVnDate(e.joinDate), true) + D("관리 대상 · HR-managed", e.hrManaged ? "예 (hrManaged)" : "아니오 · No")
      ...
      + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px">'
      + D("급여 유형 · Salary type", e.salaryType) + D("계약 급여 · Contract salary", hrFmt(e.salary) + " VND", true)
      + D("사회보험 · Social insurance", e.si ? "가입 · Enrolled" : "미가입(수습) · Not enrolled (probation)") + D("PIT 방식", e.pitMethod === "10%" ? "10% 원천 · 10% withholding" : "누진 · Progressive")
      + D("수습 기간 · Probation period", e.probEnd ? hrVnDate(e.probStart) + " ~ " + hrVnDate(e.probEnd) : "—", true)
      + D("수습 급여율 · Probation rate", e.status === "probation" ? Math.round((e.probPct || 1) * 100) + "%" : "—")
      + '</div></div></div></div>';
  }
  ...
  function hrEmpEditHtml(id) {
    ...
    return ''
      + '<div style="display:flex;gap:8px;align-items:center;margin-bottom:14px">'
      + '<button class="btn btn-outline" id="hrBack" style="font-size:11px;padding:5px 11px"><i class="ti ti-arrow-left"></i> 목록 · List</button>'
      + '<span style="font-size:12px;color:var(--text-3)">편집 · Edit · ' + hrEsc(e.id) + '</span></div>'
      + '<div class="form-card" style="padding:0;overflow:hidden">'
      + '<div style="padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">'
      + '<div style="font-size:13px;font-weight:600">' + hrEsc(e.nameVi || "신규 직원 · New employee") + ' — 편집 · Edit</div>'
      + '<div style="display:flex;gap:8px">'
      + '<button class="btn btn-outline" id="hrView" style="font-size:11px;padding:5px 12px">카드 보기 · View card</button>'
      + '<button class="btn btn-dark" id="hrSave" style="font-size:11px;padding:5px 12px"><i class="ti ti-device-floppy"></i> 저장 · Save</button></div></div>'
      + '<div style="padding:20px 22px">'
      ...
      + '<div class="fgrid">'
      + '<div class="fg"><label>관리 대상 · HR-managed</label><select data-f="hrManaged"><option value="true"' + (e.hrManaged ? " selected" : "") + '>관리 대상 · HR-managed</option><option value="false"' + (!e.hrManaged ? " selected" : "") + '>제외 · Excluded</option></select></div>'
      + '<div class="fg"><label>상태 / Status</label><select data-f="status"><option value="probation"' + (e.status === "probation" ? " selected" : "") + '>수습 · Probation</option><option value="active"' + (e.status === "active" ? " selected" : "") + '>정식 · Official</option><option value="resigned"' + (e.status === "resigned" ? " selected" : "") + '>퇴사 · Resigned</option></select></div>'
      ...
      + '<div class="fg"><label>생년월일 · Date of birth</label><input type="date" data-f="dob" value="' + hrEsc(e.dob) + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>성별 · Gender</label><select data-f="gender"><option value="M"' + (e.gender === "M" ? " selected" : "") + '>남 · Male</option><option value="F"' + (e.gender === "F" ? " selected" : "") + '>여 · Female</option></select></div>'
      + '<div class="fg"><label>CCCD 번호</label><input data-f="cccd" value="' + hrEsc(e.cccd) + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>발급일 · Issue date</label><input type="date" data-f="issued" value="' + hrEsc(e.issued) + '" style="font-family:var(--mono)"></div>'
      ...
      + '<div class="fg"><label>입사일 · Join date</label><input type="date" data-f="joinDate" value="' + hrEsc(e.joinDate) + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>수습 시작 · Probation start</label><input type="date" data-f="probStart" value="' + hrEsc(e.probStart || "") + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>수습 종료 · Probation end</label><input type="date" data-f="probEnd" value="' + hrEsc(e.probEnd || "") + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>수습 급여율 · Probation rate</label><select data-f="probPct"><option value="1"' + (e.probPct === 1 ? " selected" : "") + '>100%</option><option value="0.85"' + (e.probPct === 0.85 ? " selected" : "") + '>85%</option></select></div>'
      + '</div>'
      ...
      + '<div class="fgrid">'
      + '<div class="fg"><label>급여 유형 · Salary type</label><select data-f="salaryType"><option' + (e.salaryType === "NET" ? " selected" : "") + '>NET</option><option' + (e.salaryType === "Gross" ? " selected" : "") + '>Gross</option></select></div>'
      + '<div class="fg"><label>급여 (VND)</label><input type="number" data-f="salary" value="' + (e.salary || 0) + '" style="font-family:var(--mono)"></div>'
      + '<div class="fg"><label>사회보험 · Social insurance</label><select data-f="si"><option value="true"' + (e.si ? " selected" : "") + '>가입 · Enrolled</option><option value="false"' + (!e.si ? " selected" : "") + '>미가입 · Not enrolled</option></select></div>'
      ...
      + '<div class="fg"><label>부양가족 · Dependents</label><input type="number" data-f="dependents" value="' + (e.dependents || 0) + '" style="font-family:var(--mono)"></div>'
      + '</div></div></div>';
  }
  ...
  window.renderHrIdDocs = async function(empId) {
    ...
    var cards = list.map(function(d) {
      ...
      return '<div style="border:1px solid var(--border);border-radius:9px;overflow:hidden;background:var(--surface)">'
        ...
        + '<div style="font-size:10.5px;color:var(--text-3);margin-bottom:6px">' + hrEsc(d.label || "신분증 · ID") + ' · ' + Math.round((d.size || 0) / 1024) + ' KB</div>'
        + '<div style="display:flex;gap:6px">'
        + '<button class="btn btn-outline hr-id-dl" data-ref="' + hrEsc(d.fileRef) + '" data-name="' + hrEsc(d.name) + '" style="font-size:10px;padding:3px 8px">다운로드 · Download</button>'
        + '<button class="btn btn-outline hr-id-del" data-fid="' + hrEsc(d.fileId) + '" data-ref="' + hrEsc(d.fileRef) + '" style="font-size:10px;padding:3px 8px;color:var(--danger)">삭제 · Delete</button>'
        + '</div></div></div>';
    }).join("");
    host.innerHTML =
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-bottom:10px">'
      + (cards || '<div style="font-size:12px;color:var(--text-3);grid-column:1/-1">첨부된 신분증이 없습니다. · No ID documents attached.</div>')
      + '</div>'
      ...
      + '<option value="CCCD 앞면">CCCD 앞면 · Front</option><option value="CCCD 뒷면">CCCD 뒷면 · Back</option><option value="여권">여권 · Passport</option><option value="기타">기타 · Other</option></select>'
      + '<input type="file" id="hrIdFile" accept="image/*,.pdf" style="font-size:12px">'
      + '<button class="btn btn-dark" id="hrIdAdd" disabled style="font-size:11px;padding:5px 12px">첨부 · Attach</button>'
      + '</div>'
      + '<p style="font-size:11px;color:var(--text-3);margin-top:8px;line-height:1.5">이미지는 자동 JPEG 압축 후 Firebase Storage에 저장됩니다. RTDB에는 메타+경로만 보관합니다. · Images are auto-compressed to JPEG and stored in Firebase Storage; only metadata and the path are kept in RTDB.</p>';
    var pick = null;
    ...
    host.querySelectorAll(".hr-id-del").forEach(function(b) {
      b.onclick = async function() {
        if (!confirm("이 신분증 파일을 삭제할까요? · Delete this ID document?")) return;
